...
time_t getUsableTime(int filetime, int filedate)
{
#ifdef NDS
	struct tm tim;
	if (filetime != 0) {
		tim.tm_sec = (filetime & 0x1F) << 1;
		tim.tm_min = (filetime >> 5) & 0x3F;
		tim.tm_hour = (filetime >> 11) & 0x1F;
	}
	tim.tm_mday = filedate & 0x1F;
	tim.tm_mon = ((filedate >> 5) & 0xF) - 1; /* Make it 0-based, rather than 1-based. */
	tim.tm_year = ((filedate >> 9) & 0x7F) + 80; /* Number of years starting from 1980 onwards.  Make it from 1900 */
	return mktime(&tim);
#else
	return 0;
#endif
...
DIR_ENT FAT_DirEntFromPath (const char* path)
{
	...
	while (!notFound && !found)
	{
		...
		if (found && (path[pathPos] != '\0'))
		{
			if ((dirEntry.attrib & ATTRIB_DIR) == ATTRIB_DIR)
				// It has found a directory from within the path that needs to be followed
			{
				...
				// The matched part of the path is not a directory, but the path treats it as one.
				// If we don't explicitly accept the file is not found, then we get this current file.
			{
				notFound = true;
			}
		}
...
bool FAT_dirent_stat (DIR_ENT *dirent, struct stat *st);
bool FAT_stat (const char* path, struct stat *st)
{
	DIR_ENT dirent;

	// Get the dirEntry for the path specified
	dirent = FAT_DirEntFromPath (path);
	return FAT_dirent_stat(&dirent, st);
}
...
FAT_FILE* file: IN handle of the file to close
stat *st: IN handle of the stat structure to populate
bool return OUT: true if successful, false if not
-----------------------------------------------------------------*/
bool FAT_fstat (FAT_FILE* file, struct stat *st)
{
	DIR_ENT *dirent;
	...
	disc_ReadSector (file->dirEntSector, globalBuffer);
	dirent = &((DIR_ENT*)globalBuffer)[file->dirEntOffset];
	return FAT_dirent_stat(dirent, st);
}
...
bool FAT_dirent_stat (DIR_ENT *dirent, struct stat *st)
{
	...
	st->st_dev = FindDevice("cf0");
	st->st_ctime = getUsableTime(dirent->cTime, dirent->cDate);
	st->st_mtime = getUsableTime(dirent->mTime, dirent->mDate);
	st->st_atime = getUsableTime(0, dirent->aDate);
	st->st_size = dirent->fileSize;

	if (dirent->attrib & ATTRIB_DIR)
		st_mode |= _IFDIR;
	else
		st_mode |= _IFREG;
	st->st_mode = st_mode;
